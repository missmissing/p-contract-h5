...
<template>
  <el-table v-show="items[0].visible" :show-header="showHeader" :data="items" row-key="type" :expand-row-keys="expandkeys" class="wp100">
          ...
          <el-table :data="scope.row.financeMores">
            ...
            <el-table-column prop="paymentAmount" label="付款金额" width="150">
              <template slot-scope="scope1">
                <el-input :disabled="true" v-model="scope1.row.paymentAmount"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="paymentTimePeriod" label="付款条件" width="200">
              <template slot-scope="scope1">
                <el-select :disabled="true" v-model="scope1.row.paymentTimePeriod" placeholder="请选择付款条件">
            ...
            <el-table-column
              ...
              label="付款节点说明">
              <template slot-scope="scope1">
                <el-input :disabled="true" type="textarea" :rows="2" v-model="scope1.row.remark"></el-input>
              </template>
            </el-table-column>
    ...
    <el-table-column prop="paymentAmount" label="付款金额" width="150">
      <template slot-scope="scope">
        <el-input v-if="!scope.row.seriousPayments" :disabled="true" v-model="scope.row.paymentAmount"></el-input>
        <template v-else>{{totalPaymentAmount}}</template>
      </template>
    </el-table-column>
    <el-table-column prop="paymentTimePeriod" label="付款条件" width="200px">
      <template slot-scope="scope">
        <el-select :disabled="true" v-model="scope.row.paymentTimePeriod" placeholder="请选择付款条件">
    ...
    <el-table-column prop="remark" label="付款节点说明">
      <template slot-scope="scope">
        <el-input :disabled="true" type="textarea" :rows="2" v-model="scope.row.remark"></el-input>
      </template>
    </el-table-column>
    <el-table-column prop="ratio" label="占比" width="100">
      <template slot-scope="scope">
