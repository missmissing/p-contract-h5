...
<template>
  <el-table v-show="items[0].visible" :show-header="showHeader" :data="items" row-key="type" :expand-row-keys="expandkeys" class="wp100">
    <el-table-column type="expand" v-if="items.length&&items[0].seriousPayments">
      <template slot-scope="scope">
        <div v-if="items.length&&items[0].seriousPayments">
          <el-button v-if="!disabledTable" size="small" prefix-icon="el-icon-plus" type="primary" @click="handleAddItem(scope.row.type)" class="mb10">
            添加
          </el-button>
          <el-table :data="scope.row.financeMores">
            ...
            <el-table-column prop="paymentAmount" label="付款金额" width="150">
              <template slot-scope="scope1">
                <el-input :disabled="disabledTable" v-model="scope1.row.paymentAmount" @blur="inputChange(scope1.row)"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="paymentTimePeriod" label="付款条件" width="200">
              <template slot-scope="scope1">
                <el-select :disabled="disabledTable" v-model="scope1.row.paymentTimePeriod" placeholder="请选择付款条件">
            ...
            <el-table-column
              ...
              label="付款节点说明">
              <template slot-scope="scope1">
                <el-input :disabled="disabledTable" type="textarea" :rows="2" v-model="scope1.row.remark"></el-input>
              </template>
            </el-table-column>
            ...
            <el-table-column v-if="!disabledTable" fixed="right" label="操作" width="100">
              <template slot-scope="scope1">
                <el-button @click="handleRemove(scope1.$index, scope.row.financeMores)" type="danger" size="small">移除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-table-column prop="paymentAmount" label="付款金额" width="150">
      <template slot-scope="scope">
        <el-input v-if="!scope.row.seriousPayments" :disabled="disabledTable" v-model="scope.row.paymentAmount" @blur="inputChange(scope.row)"></el-input>
        <template v-else>{{totalPaymentAmount}}</template>
      </template>
    </el-table-column>
    <el-table-column prop="paymentTimePeriod" label="付款条件" width="200px">
      <template slot-scope="scope">
        <el-select :disabled="disabledTable||scope.row.seriousPayments" v-model="scope.row.paymentTimePeriod" placeholder="请选择付款条件">
    ...
    <el-table-column prop="remark" label="付款节点说明">
      <template slot-scope="scope">
        <el-input :disabled="disabledTable||scope.row.seriousPayments" type="textarea" :rows="2" v-model="scope.row.remark"></el-input>
      </template>
    </el-table-column>
    <el-table-column prop="ratio" label="占比" width="100">
      <template slot-scope="scope">
    ...
    <el-table-column label="操作" v-if="ifEnableRemove" width="100">
      <template slot-scope="scope">
